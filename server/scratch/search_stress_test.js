...
async function runSearchStressTest() {
  ...
  for (const test of testCases) {
    try {
      ...
      const res = await fetch(`${BASE_URL}/products/all?search=${encodeURIComponent(test.query)}&limit=5`);
      ...
      if (data.success) {
        ...
        results.push({
          ...
          Top_Results: foundNames.substring(0, 50) + (foundNames.length > 50 ? '...' : ''),
          Time_ms: duration,
          Status: products.length > 0 ? '✅' : (test.query === 'nonexistentproduct123' ? '✅' : '❌')
        });
      } else {
        results.push({ Query: test.query, Description: test.description, Status: 'ERROR', message: data.message });
      }
    } catch (error) {
      results.push({ Query: test.query, Description: test.description, Status: 'FAILED', message: error.message });
    }
  }
