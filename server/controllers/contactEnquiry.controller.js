...
export const getContactEnquiries = async (req, res, next) => {
  try {
    ...
    const contactEnquiries = await ContactEnquiry.find(dbQuery).sort({
      createdAt: -1,
    });

    res.status(200).json({ success: true, contactEnquiries });
  } catch (error) {
    next(error);
  }
};
...
export const deleteContactEnquiry = async (req, res, next) => {
  try {
    const contactEnquiry = await ContactEnquiry.findByIdAndDelete(req.params.id);
